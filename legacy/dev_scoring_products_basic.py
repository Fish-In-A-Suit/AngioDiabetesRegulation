...
def _score_term_enums_v1(terms):
    """
...
def _score_term_enums_v2(terms, process_list, term_list):
    """
    Finds which arrays the terms belong to in constants.py
    Returns the following set:
    {
...
def score_genes_v2(product_list, process_list, term_list, destination_filename="product_scores.json" , source_folder=constants.TARGET_FOLDER, use_cross_section=False):
    ...
    for term in product_list:
        if "products" in term: # key check to avoid metadata element
            if term["products"] != []: 
    ...
    for gene in gene_set:
        ...
        term_enum_score_set = _score_term_enums_v2(gene_count_result[1], process_list, term_list) # add nterms_angio, nterms_dia, nterms_angio+, nterms_angio-, nterms_angio0, nterms_dia+, nterms_dia-, nterms_dia0
        al_score = _score_gene_al_v2(term_enum_score_set, process_list)
        al_e_score = _score_gene_al_e_v2(term_enum_score_set, process_list)
        al_corr_score = _score_gene_al_corr_v2(term_enum_score_set, process_list)
...
def score_genes_v1(destination_filename, source_folder=constants.TARGET_FOLDER, use_cross_section=False):
    # this represents the function score_genes_v2 in dev_scoring_basic.py as in the commit on Nov 30, 2022 by Ladi Smrkolj
    """
    Counts the number of appearances of all the genes across all specified json_files (which contain genes
    related to specific GO terms and are made by the find_genes_related_to_GO_terms function)

    Saves the results in constants.TARGET_FOLDER/destination_filename.json
    ...
    for gene in gene_set:
        #destination_file = f"{destination_folder}/{gene}.json"
        gene_count_result = _score_gene_basic(gene,term_genes)
        term_enum_score_set = _score_term_enums_v1(gene_count_result[1]) # add nterms_angio, nterms_dia, nterms_angio+, nterms_angio-, nterms_angio0, nterms_dia+, nterms_dia-, nterms_dia0
        al_score = _score_gene_al_v1(term_enum_score_set)
        al_e_score = _score_gene_al_e_v1(term_enum_score_set)
        al_corr_score = _score_gene_al_corr_v1(term_enum_score_set)
        out = {"product": gene, "count": gene_count_result[0], "terms:": gene_count_result[1], "term_enum_scores": term_enum_score_set, "al_score": al_score,  "al_e_score": al_e_score, "al_corr_score": al_corr_score}
        if use_cross_section == True:
    ...
    json_gene_scores = util.sort_list_of_dictionaries(json_gene_scores, "al_corr_score")
    util.save_json(json_gene_scores, os.path.join(constants.TARGET_FOLDER, f"{destination_filename}.json"))
    logger.info (f"Done with scoring!")



def score_genes_v0(allowed_term_ids, destination_file, source_folder="term_genes", use_cross_section=False):
...
def _score_gene_al_v2(term_scores, process_list):
    """
...
def _score_gene_al_v1(term_scores):
    """
    With the current setting, top genes will be those which have positive angiogenetic and diabetic terms.
    Score is lower for genes with negative angiogenetic and diabetic terms.
    """
...
def _score_gene_al_e_v2(term_scores, process_list):
    score = (
...
def _score_gene_al_e_v1(term_scores):
        score = (
...
def _score_gene_al_corr_v2(term_scores, process_list):
    wanted_sign = {}
    opposite_sign = {}
    for process in process_list:
        wanted_sign[process["name"]] = process["direction"]
...
def _score_gene_al_corr_v1(term_scores):
    score = (
...
def main(use_legacy=False):
    if use_legacy == True:
        ...
        dest_filename = "product_scores" #.json will be appended in the function
        score_genes_v1(dest_filename, use_cross_section=True)

        #util.scoring_results_postprocess(os.path.join(constants.ROOT_FOLDER, f"{dest_filename}.json"))
